            ...
            try
            {
                ...
                if (IsAdmin == false)
                {
                    ...
                    if (gridData != null && gridData.Count() > 0 && oDOList != null && oDOList.Count() > 0)
                    {
                        ...
                        gridData.Where(z => z.Status.ToUpper() != "NEW").Where(z => z.cordinatorstatus.ToUpper() != "NEW").Where(z => oDOList.Any(x => x.s_DisplayProject_ID.ToUpper().Trim() != z.s_Display_Project_ID.ToUpper().Trim())).ToList().ForEach(i => i.Status = "View");
                        gridData.Where(z => z.Status.ToUpper() != "NEW").Where(z => z.cordinatorstatus.ToUpper() != "NEW").Where(z => oDOList.Any(x => x.s_DisplayProject_ID.ToUpper().Trim() == z.s_Display_Project_ID.ToUpper().Trim())).ToList().ForEach(i => i.Status = "Edit");
